...
- **`connect-src` es la excepción deliberada**: abierta a cualquier `https:` (nunca `http:`) porque el botón 🔗 deja que el usuario elija el origen — enumerar es imposible por definición. `default-src 'none'` sigue vigente, `script-src`/`style-src`/`img-src` con listas cerradas. `tests/minified.js` lo comprueba **sobre el `<meta>`**, no con `includes` sobre el archivo entero (esos nombres también aparecen en el JS).
