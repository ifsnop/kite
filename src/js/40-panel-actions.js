...
document.addEventListener("paste", e => {
  const t = e.target;
  /* SOLO se importa si el pegado ocurre EN EL ÁRBOL. En cualquier otro
     sitio el navegador pega como siempre y la aplicación no interpreta
     nada: si alguien pega un árbol en la lista de puntos de un
     polígono, se le queda el JSON en el cuadro de texto y es cosa suya
     arreglarlo. Acotarlo así es lo que hace que "pegar aquí importa"
     sea una regla que el usuario puede tener en la cabeza.
     Ojo con el guardia: NO vale descartar todo `input`. Al pulsar una
     fila el foco pasa a SU CASILLA, que es un <input type=checkbox>
     dentro del árbol, o sea justo el estado normal desde el que se
     pega — descartarlo dejaba el pegado entre pestañas sin funcionar
     nunca (comprobado). El único campo de texto que vive dentro del
     árbol es el de renombrar.                                        */
